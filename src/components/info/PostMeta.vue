<template>
  <div class="post-info" v-if="post.author">
    <span class="username" v-if="post.author.externalId">
      {{ post.author.externalId }}</span
    >
    <span class="username" v-else>User #{{ post.author.id }}</span>
    <span class="created" v-if="post.created">
      on {{ formatTimestamp(post.created) }}</span
    >
  ...
  methods: {
    ...
    formatEditTimestamp(created, edit) {
      ...
      if (createdate.getDate() === editdate.getDate()) {
        return `last edit at ${editdate.toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit',
        })}`;
      }
      return `edited on ${editdate.toLocaleDateString()}at${editdate.toLocaleTimeString(
        [],
