...
import d3annotation__default, { annotationBadge, annotationCalloutCircle, annotationCalloutRect, annotation as annotation$1 } from 'd3-svg-annotation';
import JSum from 'jsum';
import VegaEmbed from 'vega-embed';
import { select, selectAll } from 'd3';
import lodash from 'lodash';
import { EditorContent, Editor, EditorMenuBar } from 'tiptap';
import { Blockquote, BulletList, CodeBlock, HardBreak, HorizontalRule, ListItem, OrderedList, TodoItem, TodoList, Bold, Code, Italic, Link, Strike, Underline, History } from 'tiptap-extensions';
...
var script$1 = {
  ...
  methods: {
    ...
    drawAnnotations: function drawAnnotations() {
      var svg = select(this.$refs.chart.getElementsByTagName('svg')[0]); // CLEAR OLD ANNOTATIONS

      this.removeAnnotations(); // TEMP ANNOTATIONS

      if (this.tempAnnotations.length > 0) {
        this.svgAnnotations.tempAnnotations = this.makeAnnotations(this.tempAnnotations, annotationBadge);
      ...
      selectAll('.annotations').each(function (items) {
    ...
    makeAnnotations: function makeAnnotations(annotations, annotationType) {
      ...
      return annotation$1().editMode(this.annotationOptions.editMode).type(annotationType).accessors({
        x: function x(d) {
    ...
    removeAnnotations: function removeAnnotations() {
      selectAll('.annotation-group-temp').remove();
      selectAll('.annotation-group-points').remove();
      selectAll('.annotation-group-rectangles').remove();
    },
...
var __vue_staticRenderFns__$8 = [];
  ...
  /* scoped */
  const __vue_scope_id__$8 = "data-v-0d294ab6";
  /* module identifier */
...
var script$9 = {
  ...
  data: function data() {
    return {
      editor: new Editor({
        extensions: [new Blockquote(), new BulletList(), new CodeBlock(), new HardBreak(), new HorizontalRule(), new ListItem(), new OrderedList(), new TodoItem(), new TodoList(), new Bold(), new Code(), new Italic(), new Link(), new Strike(), new Underline()],
        editable: false,
...
/* template */
var __vue_render__$9 = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('div',[_c('b-card',{staticClass:"mb-2 ml-4"},[(
        _vm.comment.author &&
...
var script$a = {
  ...
  data: function data() {
    return {
      editor: new Editor({
        extensions: [new Blockquote(), new BulletList(), new CodeBlock(), new HardBreak(), new HorizontalRule(), new ListItem(), new OrderedList(), new TodoItem(), new TodoList(), new Bold(), new Code(), new Italic(), new Link(), new Strike(), new Underline(), new History()],
        content: this.comment.body
      })
    };
  },
...
var install = function install(Vue, config) {
  ...
  Vue.component('font-awesome-layers-text', FontAwesomeLayersText); // Exports plugin settings and store to components
